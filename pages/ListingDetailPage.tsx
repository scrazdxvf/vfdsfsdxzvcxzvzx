...
const ListingDetailPage: React.FC = () => {
  ...
  const { getListingById, loadingListings, deleteListing } = useListings();
  const { user } = useAuth();
  ...
  useEffect(() => {
    if (id && !loadingListings) {
      ...
      if (foundListing) {
        ...
        // Handle listing not found, e.g., navigate to a 404 page or home
        navigate('/', { replace: true });
      }
    }
  }, [id, getListingById, loadingListings, navigate]);
